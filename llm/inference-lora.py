...
def main(model_id: str, dataset_paths: list[str], destination_path: str, lora_path: str, verbose: bool = False):
    ...
    peft_model = PeftModel.from_pretrained(
        original_model, lora_path, torch_dtype=torch.float16, is_trainable=False)
    peft_model.eval()
    answers = []
    scores = []
    detailed_answers = []
    for question in tqdm(dataset, desc="Inference"):
        ...
        with torch.no_grad():
            outputs = peft_model.generate(
                input_ids=inputs["input_ids"], attention_mask=inputs["attention_mask"], max_new_tokens=2048)
            peft_model_output = tokenizer.batch_decode(
...
if __name__ == "__main__":
    ...
    main(model_id=args.model_id, dataset_paths=args.dataset_paths,
         destination_path=args.destination_path, verbose=args.verbose, lora_path=args.lora_path)
