    ...
    for _ , dataset_row in dataset_df.iterrows():
        ...
        if model_row["open"] is False:
            if model_row["model"].startswith("gpt"):
                subprocess.run(['python', 'inference-openai.py', '-i',f'{os.path.join(os.path.dirname(__file__) ,dataset_row["path"] ) }' ,'-d',f'./{dataset_row["dataset_name"]}-result-{model_row["short_name"]}.csv' ,'-m' ,f'{model_row["model"]}'])
            else:
